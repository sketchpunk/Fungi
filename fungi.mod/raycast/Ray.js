...
class Ray{
	...
	// Create Ray Based on Screen Mouse X,Y
		...
		static MouseSegment(ix,iy){
			...
			Mat4.mult(matWorld, Fungi.camera.localMatrix, Fungi.camera.invertedProjectionMatrix); //save a step by doing it backwards.
...
class BoundingBox{
	...
	update(){
		this.localBounds[0].add(this.target._position, this.worldBounds[0]);
		this.localBounds[1].add(this.target._position, this.worldBounds[1]);
		return this;
	}
}
