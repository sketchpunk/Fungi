...
class IKChainAnimator{
	...
	next(deltaTime = 0){ 
		this.cycle.update( (deltaTime)? deltaTime : Fungi.deltaTime );
		return this;
	}

	applyTarget( target, offset = 0 ){
		let n = IKChainAnimator[ this.rotateCurve ]( this.cycle.get( this.rotateCycleOffset + offset ) );
		let q = new Quat();
		...
		//-------------------------------------
		n	= IKChainAnimator[ this.scaleCurve ]( this.cycle.get( this.scaleCycleOffset + offset ) );
...
class Cycle{
	constructor(sec=1){
		...
		this.infinite		= true;
		this.spc 			= 0;		// Seconds Per Cycle
		this.setBySeconds(sec);
	}

	setBySeconds(s){ 
		this.spc 		= s;
		this.cycleInc	= Maths.PI_2 / ( s * 1000 );
		//console.log(this.cycleInc);
		return this;
	}
