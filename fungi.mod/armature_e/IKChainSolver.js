...
class IKChainSolver{
	...
	//TODO - Rename this function to Prismatic
	static piston(chain, target, pose){
		...
		let i, targetLength = chain.length * scale;

		for(i=0; i < chain.count; i++){
			if(chain.links[i].com.Bone.length >= targetLength){ 
	...
	static threeBone(chain, target, pose){
		...
		// TODO, create a ratio for the  middle bone length, use 0 and 2 total length to help divide
		// the middle bone between the upper and lower triangle. So if bone 0 is smaller then bone2, then it
		// should use less of bone1's length to solve its angle.
	...
	static chainEndPosition( chain, pose, chainPosition = null, out = null ){
		...
		if(chainPosition == null) chainPosition = TNode.getWorldPosition( chain.links[0] );
